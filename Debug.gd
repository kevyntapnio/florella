...
func _add_debug_rect(object, anchor_cell):
	...
	rect.global_position = GridManager.get_world_position(anchor_cell + Vector2i(1, 1))
	
	var occupied_tiles = object.get_occupied_tiles(anchor_cell)
	
	for tile in occupied_tiles:
		var tile_rect = ColorRect.new()
		object.add_child(tile_rect)
		tile_rect.global_position = GridManager.get_world_position(tile)
		tile_rect.modulate = Color(0.996, 0.518, 0.729, 0.549)
		tile_rect.size = Vector2i(32, 32)

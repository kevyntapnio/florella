...
func _update_tile(cell: Vector2i) -> void:
	...
	if mask == 0:
		terrain_layer.set_cell(cell, DISPLAY_SOURCE_ID, NO_TERRAIN)
		return
	...
	terrain_layer.set_cell(cell, DISPLAY_SOURCE_ID, TERRAIN_MASK_TO_ATLAS[mask])

func _get_mask(cell: Vector2i) -> int:
	var mask := 0

	if _is_terrain(cell + Vector2i(-1, -1)):
		mask |= 1
	if _is_terrain(cell + Vector2i(0, -1)):
		mask |= 2
	if _is_terrain(cell + Vector2i(-1, 0)):
		mask |= 4
	if _is_terrain(cell + Vector2i(0, 0)):
		mask |= 8

	return mask

func _is_terrain(cell: Vector2i) -> bool:
	return get_cell_atlas_coords(cell) == FULL_TERRAIN
